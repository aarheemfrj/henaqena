...
#
# Safe to re-run: it pulls the latest commit itself, skips steps that were
# already done on a prior run (env files, DB role/database, OWNER account),
# and reloads services in place instead of restarting from scratch.
...
# Running as root (or a different owner than the checkout) trips git's dubious-ownership
# guard on every fresh shell; fix it here so re-deploys never need a manual step.
git config --global --add safe.directory "$APP_DIR" 2>/dev/null || true

# Keep production environment files on the server. They are intentionally
# ignored by Git and must survive every pull/deploy so DATABASE_URL and the
# generated secrets are requested only on the first deployment.

# next-env.d.ts is regenerated by every `next build`, so a prior deployment always
# leaves it locally modified. Discard it (it's a tracked but auto-generated file,
# never hand-edited) so repeat deploys don't get blocked by their own last build.
git checkout -- apps/web/next-env.d.ts 2>/dev/null || true

# Check working tree is clean (excluding untracked files, already handled above)
...
if git pull --ff-only origin main; then
  ok "Pulled latest changes, now at $(git rev-parse --short HEAD)"
else
  die "git pull --ff-only failed. Resolve manually (git status / git log) before re-running."
fi
