...
class _AddActivityPageState extends State<AddActivityPage> {
  ...
  Future<void> _pickImages() async { final picked = await ImagePicker().pickMultiImage(imageQuality: 75, maxWidth: 1200); if (!mounted || picked.isEmpty) return; setState(() { selectedImages..clear()..addAll(picked.take(10)); imageCount = selectedImages.length; }); }
  void _review() { if (!(formKey.currentState?.validate() ?? false)) return; if (selectedImages.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('أضف صورة واضحة واحدة على الأقل للنشاط'))); return; } setState(() => preview = true); }
  ...
  Future<void> _submit() async { try { final category = categoryId; if (category == null) return; final resolvedArea = areaId ?? (await areas).first.id; final uploadedImages = await api.uploadProviderImages(selectedImages); await api.submitProvider(data: {'name': name.text.trim(), 'description': description.text.trim(), 'phone': phone.text.trim(), 'whatsapp': whatsapp.text.trim().isEmpty ? null : whatsapp.text.trim(), 'phoneType': phoneType, 'serviceMode': mode, 'areaId': resolvedArea, 'categoryIds': [category], 'openingTime': opening, 'closingTime': closing, 'address': address.text.trim(), 'images': uploadedImages}); if (!mounted) return; Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم إرسال النشاط وصوره للمراجعة'))); } catch (error) { if (!mounted) return; if (error.toString().contains('unauthorized')) { await Navigator.of(context).push(MaterialPageRoute(builder: (_) => const AuthPage())); return; } ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(error.toString().contains('duplicate') ? 'النشاط موجود بالفعل أو قيد المراجعة' : error.toString().contains('upload_error') ? 'تعذر رفع الصور، جرّب صوراً أصغر' : 'تعذر إرسال النشاط حالياً'))); } }
}
