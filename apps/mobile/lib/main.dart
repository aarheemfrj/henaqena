...
class ListingDetailPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(appBar: AppBar(title: const Text('تفاصيل الإعلان'), actions: [IconButton(onPressed: () {}, icon: const Icon(Icons.share_outlined)), IconButton(onPressed: () {}, icon: const Icon(Icons.favorite_border))]), body: ListView(padding: const EdgeInsets.all(18), children: [Hero(tag: 'listing-image-$title', child: Container(height: 190, decoration: BoxDecoration(color: const Color(0xFFE8F0EE), borderRadius: BorderRadius.circular(20)), child: const Icon(Icons.image_outlined, color: teal, size: 54))), const SizedBox(height: 16), Row(children: [Expanded(child: Text(title, style: const TextStyle(color: deepTeal, fontSize: 21, fontWeight: FontWeight.w700))), const Chip(label: Text('مراجع'), avatar: Icon(Icons.verified_outlined, size: 16, color: teal), backgroundColor: Color(0xFFE8F5F2))]), const SizedBox(height: 8), Text('$price · $location', style: const TextStyle(color: muted)), const SizedBox(height: 20), Row(children: [Expanded(child: FilledButton.icon(onPressed: () {}, icon: const Icon(Icons.phone_outlined), label: const Text('اتصال'))), const SizedBox(width: 8), Expanded(child: OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.chat_outlined), label: const Text('واتساب')))]), const SizedBox(height: 18), Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [TextButton.icon(onPressed: () {}, icon: const Icon(Icons.thumb_up_alt_outlined), label: const Text('مهتم')), TextButton.icon(onPressed: () {}, icon: const Icon(Icons.bookmark_border), label: const Text('حفظ')), TextButton.icon(onPressed: () {}, icon: const Icon(Icons.ios_share_outlined), label: const Text('مشاركة'))]), const SizedBox(height: 10), const SectionTitle(title: 'الوصف'), const SizedBox(height: 8), const Text('إعلان منشور من مستخدم بعد مراجعة الإدارة. التفاصيل والصور قابلة للتحديث من صاحب الإعلان.', style: TextStyle(color: muted, height: 1.5)), const SizedBox(height: 22), const SectionTitle(title: 'التفاعل'), const SizedBox(height: 8), const MiniItem(icon: Icons.person_outline, title: 'أحمد محمد', subtitle: 'إعلان واضح ومعلوماته كاملة · منذ ساعة'), const SizedBox(height: 10), OutlinedButton.icon(onPressed: () => _report(context), icon: const Icon(Icons.flag_outlined), label: const Text('إبلاغ عن الإعلان'))])));
  void _report(BuildContext context) { showModalBottomSheet<void>(context: context, builder: (_) => Directionality(textDirection: TextDirection.rtl, child: SafeArea(child: Wrap(children: [const Padding(padding: EdgeInsets.all(18), child: Text('سبب الإبلاغ', style: TextStyle(color: deepTeal, fontSize: 18, fontWeight: FontWeight.w700))), ...['السعر غير صحيح', 'محتوى مخالف', 'إعلان مكرر', 'سبب آخر'].map((x) => ListTile(title: Text(x), leading: const Icon(Icons.radio_button_unchecked, color: teal), onTap: () => Navigator.pop(context)))])))); }
}
