...
Price confirmations are evidence about freshness, not a new publication or guaranteed quote. A user can update their own confirmation once per price; the aggregate count is shown only for approved active prices.

## مين شاطر

Questions and recommendations are submitted as `PENDING` and become public only after approval. Closed questions remain readable but reject new recommendations; rejected, archived and deleted content is private to its author/admin. A recommendation may link to an approved active provider or remain manual, but never creates a provider automatically. Helpful is idempotent and reports never auto-delete content. Anonymous posting and images remain deferred. The question owner may select one approved recommendation as the best answer; this never changes provider rating or verification.
